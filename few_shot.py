few_shot_count = '''
Here are some examples of questions and their corresponding Cypher queries for this graph which you can refer to. 

Question: How many items fall into the 18 classification?
Output: MATCH (s:Symbol) WHERE s.class = 18 RETURN count(s)

Question: Could you determine the total count of symbols of type 23?"
Output: MATCH (s:Symbol) WHERE s.class = 23 RETURN count(s)

Question: What’s the number of entities in class 7?
Output: MATCH (s:Symbol) WHERE s.class = 7 RETURN count(s)

Question: Enumerate distinct symbols of class 32
Output: MATCH (s:Symbol) WHERE s.class = 32 RETURN count(s)

Question: Give the number of symbols listed under 11?
Output: MATCH (s:Symbol) WHERE s.class = 11 RETURN count(s)
'''


few_shot_spatial_count = '''
Here are some examples of questions and their corresponding Cypher queries for this graph which you can refer to.

Question: How many elements in class 21 are directly associated with elements in class 5?
Output: MATCH (s1:Symbol)-[:CONNECTED_TO]-(s2:Symbol) WHERE s1.class = 21 AND s2.class = 5 RETURN COUNT(s1)

Question: What’s the total number of 11 symbols connected to 7 symbols?
Output: MATCH (s1:Symbol)-[:CONNECTED_TO]-(s2:Symbol) WHERE s1.class = 11 AND s2.class = 7 RETURN COUNT(s1)

Question: How many entities belonging to 16 are linked directly to entities in 7?
Output: MATCH (s1:Symbol)-[:CONNECTED_TO]-(s2:Symbol) WHERE s1.class = 16 AND s2.class = 7 RETURN COUNT(s1)

Question: Can you provide the count of 31-class symbols connected directly to 28-class symbols?
Output: MATCH (s1:Symbol)-[:CONNECTED_TO]-(s2:Symbol) WHERE s1.class = 31 AND s2.class = 28 RETURN COUNT(s1)

Question: What’s the number of direct links between symbols of class 23 and class 5?
Output: MATCH (s1:Symbol)-[:CONNECTED_TO]-(s2:Symbol) WHERE s1.class = 23 AND s2.class = 5 RETURN COUNT(s1)
'''

few_shot_spatial_connections = '''
Here are some examples of questions and their corresponding Cypher queries for this graph which you can refer to.

Question: Are there any symbols classified as 1 positioned between symbols of classes 2 and 3?
Output: MATCH (s1:Symbol)-[:CONNECTED_TO]-(s2:Symbol)-[:CONNECTED_TO]-(s3:Symbol) WHERE s1.class = 2 AND s2.class = 1 AND s3.class = 7 RETURN COUNT(s2) > 0


Question: Could you check if a symbol of class 8 serves as a link between class 19 and class 13?
Output: MATCH (s1:Symbol)-[:CONNECTED_TO]-(s2:Symbol)-[:CONNECTED_TO]-(s3:Symbol) WHERE s1.class = 19 AND s2.class = 8 AND s3.class = 13 RETURN COUNT(s2) > 0

Question:Can you find any symbol in class 21 that connects symbols of classes 8 and 10 at the same time?
Output: MATCH (s1:Symbol)-[:CONNECTED_TO]-(s2:Symbol)-[:CONNECTED_TO]-(s3:Symbol) WHERE s1.class = 8 AND s2.class = 21 AND s3.class = 10 RETURN COUNT(s2) > 0

Question: Can you determine if symbols of class 10 are situated between those of class 4 and class 12?
Output: MATCH (s1:Symbol)-[:CONNECTED_TO]-(s2:Symbol)-[:CONNECTED_TO]-(s3:Symbol) WHERE s1.class = 4 AND s2.class = 10 AND s3.class = 12 RETURN COUNT(s2) > 0

Question: Is it possible to identify symbols in class 29 on one side and class 17 on another side of class 25 symbol?
Output: MATCH (s1:Symbol)-[:CONNECTED_TO]-(s2:Symbol)-[:CONNECTED_TO]-(s3:Symbol) WHERE s1.class = 29 AND s2.class = 25 AND s3.class = 17 RETURN COUNT(s2) > 0
'''

few_shot_value_based = '''
Here are some examples of questions and their corresponding Cypher queries for this graph which you can refer to.

Question: Can you provide a list of tags for symbols in class 8 where the tags start with ZX?
Output: MATCH (s:Symbol)WHERE s.class = 8 AND s.tag STARTS WITH 'ZX' RETURN s.tag

Question: What are all the tags of class 7 symbols that have QW as their prefix?
Output: MATCH (s:Symbol) WHERE s.class = 7 AND s.tag STARTS WITH 'QW' RETURN s.tag

Question: Could you list the tags for class 22 symbols that begin with the string YKX?
Output: MATCH (s:Symbol) WHERE s.class = 22 AND s.tag STARTS WITH 'YKX' RETURN s.tag

Question: Fetch the tags of 28 symbols where each tag starts with the prefix MS.
Output: MATCH (s:Symbol) WHERE s.class = 28 AND s.tag STARTS WITH 'MS' RETURN s.tag

Question: Can you return the tags of all class 14 symbols that start with FU as their prefix?
Output: MATCH (s:Symbol) WHERE s.class = 14 AND s.tag STARTS WITH 'FU' RETURN s.tag
'''
